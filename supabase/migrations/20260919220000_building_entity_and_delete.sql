-- Migration: Building Entity Ownership + Delete Records
-- 1. Add entity_id to buildings (ownership flows Entity → Building → Unit)
-- 2. Populate building entity_id from existing unit data
-- 3. Add lease_delete_record RPC for hard deletes
-- 4. Update lease_load_dataset to include building entityId
-- 5. Update lease_save_record for building entityId + auto-derive unit entityId
-- 6. Remove duplicate unit

-- ============================================================
-- Part 1: Add entity_id to lease_buildings
-- ============================================================

alter table public.lease_buildings
  add column if not exists entity_id text references public.lease_entities(id) on delete restrict;

-- Populate from the most common entity among each building's units
update public.lease_buildings b
set entity_id = sub.entity_id
from (
  select distinct on (building_id) building_id, entity_id
  from public.lease_units
  where is_active
  group by building_id, entity_id
  order by building_id, count(*) desc
) sub
where b.id = sub.building_id and b.entity_id is null;

-- For buildings with no units yet, leave entity_id null (user must set it)

-- ============================================================
-- Part 2: Delete record RPC
-- ============================================================
...
begin
  ...
  case collection_name
    ...
    when 'buildings' then
      select count(*) into ref_count from public.lease_units where building_id = record_id and is_active;
      if ref_count > 0 then
        raise exception using errcode = '23503', message = 'DELETE_BLOCKED: this building still has active units. Archive or delete them first.';
      end if;
    ...
    when 'entities' then
      select count(*) into ref_count from public.lease_buildings where entity_id = record_id and is_active;
      if ref_count > 0 then
        raise exception using errcode = '23503', message = 'DELETE_BLOCKED: this entity still owns active buildings. Reassign or archive them first.';
      end if;
    ...
    when 'standardOptions' then
      select count(*) into ref_count from public.lease_standard_options
      where id = record_id and record_revision = expected_record_revision;
      if ref_count = 0 then
        raise exception using errcode = '40001', message = 'STALE_RECORD: option changed or no longer exists.';
      end if;
      delete from public.lease_unit_included_options where option_id = record_id;
      delete from public.lease_unit_responsibility_options where option_id = record_id;
...
-- ============================================================
-- Part 3: Update lease_load_dataset to include building entityId
-- ============================================================

create or replace function private.lease_load_dataset()
returns jsonb
language sql
stable
security definer
...
as $function$
  select jsonb_build_object(
    'schemaVersion', (select schema_version from public.lease_dataset_state where singleton),
    'revision', (select revision from public.lease_dataset_state where singleton),
    ...
    'buildings', coalesce((select jsonb_agg(jsonb_build_object(
      'id', id, 'entityId', entity_id, 'displayName', display_name, 'streetAddress', street_address,
...
-- ============================================================
-- Part 4: Update lease_save_record for building entityId
-- ============================================================
...
-- ============================================================
-- Part 5: Public RPC wrapper for delete + grant
-- ============================================================

create or replace function public.lease_admin_delete_record(
  collection_name text, record_id text,
  expected_record_revision bigint, expected_dataset_revision bigint
)
returns jsonb language sql volatile security invoker set search_path = ''
as $function$
  select private.lease_delete_record(collection_name, record_id, expected_record_revision, expected_dataset_revision)
$function$;

revoke execute on function public.lease_admin_delete_record(text,text,bigint,bigint) from public, anon;
grant execute on function public.lease_admin_delete_record(text,text,bigint,bigint) to authenticated;

-- ============================================================
-- Part 6: Bump dataset revision
-- ============================================================
