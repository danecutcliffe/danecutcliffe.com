-- Migration: Lease Manage Enhancements
-- 1. Add INSERT paths for entities, buildings, and units in lease_save_record
-- 2. Consolidate buildings per Dane's Finder directory structure
-- 3. Add missing buildings (61 Orlebar, 356-358 Queen)
-- 4. Update unit display names to standard format

-- ============================================================
-- Part 1: Replace lease_save_record to support creates
-- ============================================================
    ...
    when 'buildings' then
      ...
      if expected_record_revision = 0 then
        ...
        insert into public.lease_buildings (
          id, display_name, street_address, community, province, postal_code,
          is_active, manual_fields, updated_by
        ) values (
          record_id,
          patch->>'displayName',
    ...
    when 'units' then
      ...
      if expected_record_revision = 0 then
        if record_id !~ '^unit-[a-z0-9][a-z0-9-]*$'
           ...
           or btrim(coalesce(patch->>'entityId','')) = ''
           or btrim(coalesce(patch->>'displayName','')) = ''
           or btrim(coalesce(patch->>'premisesStreetAddress','')) = '' then
          raise exception using errcode = '22023', message = 'PATCH_INVALID: a new unit requires a valid stable ID, building, entity, display name and premises street address.';
        end if;
...
-- ============================================================
-- Part 2: Consolidate buildings
-- ============================================================

-- 2a. Rename 8 Orlebar -> 8-14 Orlebar (the surviving building)
update public.lease_buildings
set display_name = '8-14 Orlebar',
    street_address = '8-14 Orlebar Street',
    record_revision = record_revision + 1,
    updated_at = now()
where id = 'building-8-orlebar-street';

-- 2b. Re-point all Orlebar units to the consolidated building
update public.lease_units
set building_id = 'building-8-orlebar-street',
    record_revision = record_revision + 1,
    updated_at = now()
where building_id in ('building-10-orlebar-street', 'building-12-orlebar-street', 'building-14-orlebar-street');

-- 2c. Deactivate the redundant Orlebar buildings
update public.lease_buildings
set is_active = false,
    record_revision = record_revision + 1,
    updated_at = now()
where id in ('building-10-orlebar-street', 'building-12-orlebar-street', 'building-14-orlebar-street');

-- 2d. Rename 92 Upper Hillsborough Street -> 92 Upper Hillsborough
update public.lease_buildings
set display_name = '92 Upper Hillsborough',
    record_revision = record_revision + 1,
    updated_at = now()
where id = 'building-92-upper-hillsborough-street';

-- 2e. Re-point 92B unit to the consolidated building
update public.lease_units
set building_id = 'building-92-upper-hillsborough-street',
    record_revision = record_revision + 1,
    updated_at = now()
where building_id = 'building-92b-upper-hillsborough-street';

-- 2f. Deactivate the redundant 92B building
update public.lease_buildings
set is_active = false,
    record_revision = record_revision + 1,
    updated_at = now()
where id = 'building-92b-upper-hillsborough-street';

-- 2g. Rename remaining buildings to match Finder directory conventions
update public.lease_buildings
set display_name = '178 Weymouth',
    record_revision = record_revision + 1,
    updated_at = now()
where id = 'building-178-weymouth-street' and display_name <> '178 Weymouth';

update public.lease_buildings
set display_name = '20 Longworth',
    record_revision = record_revision + 1,
    updated_at = now()
where id = 'building-20-longworth-avenue' and display_name <> '20 Longworth';

-- ============================================================
-- Part 3: Add missing buildings
-- ============================================================

insert into public.lease_buildings (id, display_name, street_address, community, province, postal_code, manual_fields)
values
  ('building-61-orlebar', '61 Orlebar', '61 Orlebar Street', 'Charlottetown', 'PE', '', array['displayName','streetAddress','community','province']),
  ('building-356-358-queen', '356-358 Queen', '356-358 Queen Street', 'Charlottetown', 'PE', '', array['displayName','streetAddress','community','province'])
on conflict (id) do nothing;

-- ============================================================
-- Part 4: Update unit display names to standard format
-- ============================================================

-- Numeric unit number: "{unitNumber}-{abbreviated address}"
-- Letter unit: "{civic}{letter} {street abbrev}"
-- No unit number: "{abbreviated address}"

update public.lease_units set display_name = '3-12 Orlebar St',  record_revision = record_revision + 1, updated_at = now() where id = 'unit-3-12-orlebar-street';
update public.lease_units set display_name = '8 Orlebar St',     record_revision = record_revision + 1, updated_at = now() where id = 'unit-8-orlebar-street';
update public.lease_units set display_name = '2-12 Orlebar St',  record_revision = record_revision + 1, updated_at = now() where id = 'unit-2-12-orlebar-street';
update public.lease_units set display_name = '10 Orlebar St',    record_revision = record_revision + 1, updated_at = now() where id = 'unit-10-orlebar-street';
update public.lease_units set display_name = '1-12 Orlebar St',  record_revision = record_revision + 1, updated_at = now() where id = 'unit-1-12-orlebar-street';
update public.lease_units set display_name = '14 Orlebar St',    record_revision = record_revision + 1, updated_at = now() where id = 'unit-14-orlebar-street';
update public.lease_units set display_name = '92 Upper Hillsborough St',  record_revision = record_revision + 1, updated_at = now() where id = 'unit-92-upper-hillsborough-street';
update public.lease_units set display_name = '92B Upper Hillsborough St', record_revision = record_revision + 1, updated_at = now() where id = 'unit-92b-upper-hillsborough-street';
update public.lease_units set display_name = '2-178 Weymouth St',record_revision = record_revision + 1, updated_at = now() where id = 'unit-2-178-weymouth-street';
update public.lease_units set display_name = '3-20 Longworth Ave',record_revision = record_revision + 1, updated_at = now() where id = 'unit-3-20-longworth-avenue';

-- ============================================================
-- Part 5: Bump dataset revision to reflect data changes
-- ============================================================
