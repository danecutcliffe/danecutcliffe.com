...
function NewLease({ adapters, dataset, onDatasetChange, onManage }: {
  ...
  const selectedEntity = selectedUnit ? dataset.entities.find((entity) => entity.id === selectedUnit.entityId) : undefined;
...
function Manage({ adapters, dataset, initialTarget, onDatasetChange }: {
  ...
  return <section className="workspace manage"><div className="section-heading"><p className="eyebrow">Portfolio</p><h2>Manage</h2><p>Canonical lease defaults and safe portfolio data operations.</p></div>{message && <Notice kind={message === 'Saved.' ? 'success' : 'error'} onDismiss={() => setMessage('')}>{message}</Notice>}<div className="manage-layout"><nav className="manage-nav" aria-label="Manage sections">{(['entities','buildings','units','options','data'] as ManageSection[]).map((value) => <button key={value} className={section === value ? 'nav-active' : ''} onClick={() => { setSection(value); setRecordId(''); }}>{manageLabel(value)}</button>)}</nav><div className="manage-content">
    {section === 'entities' && <EntityEditor records={dataset.entities} selectedId={recordId} onSelect={setRecordId} onSave={(record) => saved(adapters.data.saveEntity(record))} onArchive={archiveEntity} />}
    {section === 'buildings' && <BuildingEditor records={dataset.buildings} selectedId={recordId} onSelect={setRecordId} onSave={(record) => saved(adapters.data.saveBuilding(record))} onArchive={archiveBuilding} />}
    {section === 'units' && <UnitEditor records={dataset.units} buildings={dataset.buildings} entities={dataset.entities} options={dataset.standardOptions} selectedId={recordId} onSelect={setRecordId} onSave={(record) => saved(adapters.data.saveUnit(record))} onArchive={archiveUnit} />}
...
function RecordPicker<T extends { id: string }>({ records, selectedId, onSelect, label }: { records: T[]; selectedId: string; onSelect: (id: string) => void; label: (record: T) => string }) {
  return <label>{`Choose ${label as unknown as string}`.startsWith('Choose function') ? 'Record' : 'Record'}<select value={selectedId} onChange={(event) => onSelect(event.target.value)}><option value="">Select…</option>{records.map((record) => <option key={record.id} value={record.id}>{label(record)}</option>)}</select></label>;
}

function EntityEditor({ records, selectedId, onSelect, onSave, onArchive }: { records: Entity[]; selectedId: string; onSelect: (id: string) => void; onSave: (value: Entity) => void; onArchive: (value: Entity) => void }) {
...
function BuildingEditor({ records, selectedId, onSelect, onSave, onArchive }: { records: Building[]; selectedId: string; onSelect: (id: string) => void; onSave: (value: Building) => void; onArchive: (value: Building) => void }) {
  ...
  const newBuilding = () => { const building: Building = { id: `building-${crypto.randomUUID()}`, displayName:'', streetAddress:'', community:'Charlottetown', province:'PE', postalCode:'', recordRevision: 0 }; setValue(building); onSelect(''); };
  return <Editor title="Buildings"><div className="picker-row"><RecordPicker records={records} selectedId={selectedId} onSelect={onSelect} label={(record) => record.displayName} /><button className="secondary" onClick={newBuilding}>+ New building</button></div>{value && <form onSubmit={(event) => { event.preventDefault(); onSave(value); }}><div className="form-grid two">{fields.map(([key,label]) => <label key={String(key)}>{label}<input value={String(value[key] ?? '')} onChange={(event) => setValue({ ...value, [key]: event.target.value })} /></label>)}</div><div className="editor-actions"><button className="primary">{value.recordRevision === 0 ? 'Create Building' : 'Save Building'}</button>{(value.recordRevision ?? 0) > 0 && <button type="button" className="text-button danger" onClick={() => onArchive(value)}>Archive</button>}</div></form>}</Editor>;
}

function UnitEditor({ records, buildings, entities, options, selectedId, onSelect, onSave, onArchive }: { records: Unit[]; buildings: Building[]; entities: Entity[]; options: StandardOption[]; selectedId: string; onSelect: (id: string) => void; onSave: (value: Unit) => void; onArchive: (value: Unit) => void }) {
