...
function UnitEditor({ records, buildings, entities, options, selectedId, onSelect, onSave, onArchive, onDelete }: { records: Unit[]; buildings: Building[]; entities: Entity[]; options: StandardOption[]; selectedId: string; onSelect: (id: string) => void; onSave: (value: Unit) => void; onArchive: (value: Unit) => void; onDelete: (value: Unit) => void }) {
  ...
  return <Editor title="Units">
    <label>Filter by building<select value={filterBuildingId} onChange={(event) => { setFilterBuildingId(event.target.value); onSelect(''); }}><option value="">All buildings</option>{buildings.map((b) => <option key={b.id} value={b.id}>{b.displayName}</option>)}</select></label>
    <div className="picker-row"><RecordPicker records={filteredRecords} selectedId={selectedId} onSelect={onSelect} label={(record) => filterBuildingId ? record.displayName : `${buildings.find((building) => building.id === record.buildingId)?.displayName ?? ''} · ${record.displayName}`} /><button className="secondary" onClick={newUnit}>+ New unit</button></div>{value && <form onSubmit={handleSave}>
