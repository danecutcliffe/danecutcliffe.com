...
export function AdminReports({ profiles, jobSites, jobCodes, entries, auditLogs, payPeriodSettings }: AdminReportsProps) {
  ...
  const filteredEntries = periodEntries.filter((entry) => (!employeeId || entry.userId === employeeId) && (!jobCodeId || entry.jobCodeId === jobCodeId));
  const exportableEntries = filteredEntries.filter((entry) => entry.clockOut);
  const blockers = buildExportBlockers(filteredEntries, profileById);
  const summary = calculatePayrollSummary(filteredEntries, employees);
    ...
    <section className="space-y-4">
      <div id="payroll-export" className="scroll-mt-20 rounded-md border border-app-border bg-card p-4 shadow-soft">
        ...
        <div className="mt-4 grid gap-3 sm:grid-cols-2 lg:grid-cols-5">
          <Metric label="Payable hours" value={`${summary.netWorkHours.toFixed(2)}h`} />
          <Metric label="Gross payroll" value={money(summary.grossPay)} />
          <Metric label="Overtime" value={`${summary.overtimeHours.toFixed(2)}h`} />
...
function Metric({ label, value }: { label: string; value: string }) {
  return (
    <div className="rounded-md bg-card-alt p-3">
